...
export default async function Cards() {
  try {
    ...
    return(
        <>
        {eventos.map((evento) => (
            <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white" key={evento.id}>
          <Image src="./assets/img/placeholder-curso.jpg" alt="Imagem placeholder do curso Formação em Acionamentos WEG CFW" class="h-44 w-full object-cover">
          <div class="p-5">
