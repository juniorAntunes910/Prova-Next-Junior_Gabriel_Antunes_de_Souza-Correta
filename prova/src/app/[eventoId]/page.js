...
export default function detail() {
    //Parametros que foram enviados por meio do Link utilizando a query, Utilizei desta forma pois é mais otimizada
    //Coleto todos os parametros
  const params = useSearchParams();
  //transformo em um objeto para facilitar a sua inscerção
  const userDetail = {
    id: params.get("id"),
  ...
  //Preparando a formatação de Data
  //Utilizei Diversos splits para a formatação exata de como estava no html
  ...
  //Retorno, utilizei tanto o objeto de detail quanto o de dataFormatada.
  return (
        ...
        <div className="mt-8 grid gap-8 md:grid-cols-[1.35fr_0.65fr]">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-2 text-xs font-bold text-slate-600">
              ...
              <span className="rounded-full bg-white px-3 py-1">
                {`${dataParaCurso.diaSemana}/${dataParaCurso.mes}/${dataParaCurso.ano} às ${dataParaCurso.hora}:${dataParaCurso.minutos}`}
              </span>
              <span className="rounded-full bg-white px-3 py-1">
